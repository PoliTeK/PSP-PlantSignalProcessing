...
public:
    ...
    EffectWrapper(  t_effect* effectPtr,
    ...
    inline float Process (t_process sample) {
        ...
        if (_Process_ptr == nullptr){
            ...
            processedSample = (_effectPtr->*_ProcessWithNoArgs_ptr)();
        }
        else {

            // if also this wasn't setted properly it shouldnt compile
            processedSample = (_effectPtr->*_Process_ptr) (sample);
        }
    ...
    inline void SetParam (t_param1 param1) {return (_effectPtr->*_SetParamFunction_ptr[0]) (param1);};
